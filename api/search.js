...
//
// The public unauthenticated tier is rate-limited (~1 req/sec shared across
// all users), so a 429 is expected occasionally — not a sign Semantic
// Scholar is broken.
//
// RETRY POLICY: a single short retry on 429 IS attempted, but only if
// enough budget remains (≥2.5s) to safely afford a 400ms backoff + a
// second request without risking the overall timeout. This was previously
// removed entirely because the old glm-4-flash-based pipeline had almost
// no time margin left — any retry pushed downstream /api/chat past 9s.
// Now that text generation runs on glm-4-air (faster, higher QPS tier),
// production logs show keyword translation finishing in ~1.4s with 7s+
// still remaining, so there's comfortable room for one retry attempt
// without reintroducing the original timeout risk.
async function doSemanticScholarSearch(query, timeoutMs) {
  if (timeoutMs <= 500) return [];

  const attempt = async (msBudget) => {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), msBudget);
    try {
      ...
      if (resp.status === 429) return { rateLimited: true, papers: [] };
      if (!resp.ok) {
        console.error('[semantic-scholar] HTTP error', resp.status);
        return { rateLimited: false, papers: [] };
      }
      const data = await resp.json();
  ...
  if (!first.rateLimited || first.papers.length > 0) return first.papers;

  // Rate-limited — retry once, but only if there's genuinely enough budget
  // left for a backoff + a second request. This hard check is what makes
  // the retry safe: if budget is already tight, skip it and fall through
  // to the Zhipu fallback immediately rather than risk a timeout.
  const elapsed = Date.now() - startedAt;
  const budgetLeft = timeoutMs - elapsed;
  const RETRY_BACKOFF_MS = 400;
  const MIN_BUDGET_FOR_RETRY_MS = 2500;
  if (budgetLeft < MIN_BUDGET_FOR_RETRY_MS) {
    console.warn(`[semantic-scholar] rate limited (429) — only ${budgetLeft}ms left, skipping retry, falling back to Zhipu`);
    return [];
  }
  console.warn(`[semantic-scholar] rate limited (429) — retrying once after ${RETRY_BACKOFF_MS}ms backoff (${budgetLeft}ms budget available)`);
  await new Promise(r => setTimeout(r, RETRY_BACKOFF_MS));
  const second = await attempt(budgetLeft - RETRY_BACKOFF_MS);
  if (second.rateLimited) console.warn('[semantic-scholar] rate limited again on retry — falling back to Zhipu');
...
export default async function handler(req, res) {
  ...
  try {
    if (module === 'data') {
      ...
      console.log(`[search] Semantic Scholar done, +${Date.now()-startTime}ms elapsed, papers:${papers.length}, ${remaining()}ms left`);
      if (papers.length > 0) {
