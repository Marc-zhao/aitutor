...
// Scholar is broken. IMPORTANT: we do NOT retry on 429. Production logs
// showed that every chat-generation timeout correlated exactly with a 429
// retry attempt — the 600ms backoff plus a second full request consumed
// enough of the already-tight 10s budget that downstream /api/chat ran out
// of time to generate a reply. Giving up immediately on rate-limit and
// falling through to the faster Zhipu official-tier fallback is what
// actually keeps total latency inside the budget.
async function doSemanticScholarSearch(query, timeoutMs) {
  ...
  try {
    ...
    if (resp.status === 429) {
      console.warn('[semantic-scholar] rate limited (429) — giving up immediately, falling back to Zhipu');
      return [];
    }
    if (!resp.ok) {
