...
// all users), so a 429 is expected occasionally under concurrent load — not
// a sign Semantic Scholar is broken. We retry ONCE after a short backoff,
// but only if there's enough remaining budget to make it worthwhile;
// otherwise we give up immediately rather than risk the whole function timing out.
async function doSemanticScholarSearch(query, timeoutMs) {
  ...
  // Rate-limited on first try — retry once after a brief backoff, only if
  // we have enough budget left (need at least ~1.5s for the backoff + a
  // minimal request window, otherwise it's not worth the risk).
  console.warn('[semantic-scholar] rate limited (429), retrying once after backoff');
  await new Promise(r => setTimeout(r, 600));
  return (await attempt(Math.max(0, timeoutMs - 600))).papers;
}
