...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Production incident: the model wrote "根据来源②的研究…" in reply_text
    // while cited_source_indices reported something inconsistent with what
    // was actually available (only 1 real source existed, yet the text
    // referenced a second one). The fix above (asking the model to keep
    // these consistent) is a prompt instruction — not reliable on its own,
    // per the established pattern in this codebase (see lib/guardrails.js's
    // entire rationale). This scans reply_text ITSELF for "来源③"/"source #3"
    // style mentions and independently verifies the number is within the
    // REAL source list's range, rather than trusting the model's own
    // cited_source_indices report. Any out-of-range mention triggers a
    // fallback to an honest "couldn't find a second source" message instead
    // of showing the student a reference to a source that doesn't exist.
    if (useStructuredOutput) {
      ...
      const fabricatedMention = mentionedIndices.find(n => n > realSourceList.length);
      if (fabricatedMention) {
        console.error(`[chat] AUDITOR CAUGHT FABRICATION: reply_text mentions 来源${fabricatedMention} but only ${realSourceList.length} real source(s) exist this turn. Overriding reply.`);
        content = realSourceList.length
          ? `我目前只找到了${realSourceList.length}篇相关来源——没有找到你要求的"更直接"的第二篇研究。要不要换个角度搜索，或者我们继续讨论现有这篇来源能告诉我们什么？`
